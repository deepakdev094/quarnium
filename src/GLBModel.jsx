...
export function GLBModel({ scrollY }) {
  ...
  const { scene } = useGLTF('/bitcoinObj.glb'); // Assuming the file is in /public
  const { camera } = useThree();
  
  // Store the last valid scroll position and scale
  const lastScrollPos = useRef(0);
  const lastScale = useRef(0.15); // Increased initial scale
  ...
  useFrame((state, delta) => {
    if (ref.current) {
      ...
      let scale = 0.15; // Increased base scale
      
      if (scrollProgress > 0.75) {
        // In the last quarter, zoom in gradually
        const zoomProgress = (scrollProgress - 0.75) * 4; // 0 to 1 for the last quarter
        scale = 0.15 + zoomProgress * 0.3; // Start at 0.15, end at 0.45
      } else {
        // Small fluctuations in other quarters
        scale = 0.15 + Math.sin(scrollProgress * Math.PI * 4) * 0.02; // Subtle size pulsing
      }
  ...
  return (
    <>
      ...
      <primitive
        ref={ref}
        object={scene}
        position={[2.5, 0, 0]} // Start from right side
        scale={[0.15, 0.15, 0.15]} // Increased initial scale
        castShadow
        receiveShadow
      />
      <OrbitControls 
